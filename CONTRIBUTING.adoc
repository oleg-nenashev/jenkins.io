...
Once you finish a patch in your local repository, you can create a pull request against link:https://github.com/jenkins-infra/jenkins.io[jenkins-infra/jenkins.io].
After it is created, a copy-editors team will review your pull request and provide feedback.

Some tips:

* You can incrementall push additional commits to a pull request, no need to close and recreate a pull request
* Do not squash and force-push commits to a pull request.
  It complicates the change reviews, because the commit history is lost.
  If needed, we can squash your pull request during the merge.
* There is a GitHub action which automatically compresses images in pull requests.
  It will add commits to your branch, and you may need to pull the changes.
